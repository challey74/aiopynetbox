...
All HTTP funnels through `Api._request()` ([api.py](src/apynetbox/api.py)) — auth headers (v1 `Token`/v2 `nbt_` `Bearer`), error raising, JSON decoding live there and nowhere else. `App.__getattr__` ([app.py](src/apynetbox/app.py)) turns any attribute into an `Endpoint` ([endpoint.py](src/apynetbox/endpoint.py)), which builds URLs (`_`→`-`) and returns `Record`/`RecordSet` ([response.py](src/apynetbox/response.py)). `PluginsApp` (also app.py) routes `nb.plugins.<plugin>` into `/api/plugins/<plugin>/`.

`Endpoint.__init__` resolves its Record subclass from `ENDPOINT_MODELS` in [models.py](src/apynetbox/models.py) (`"<app>/<endpoint>"` keys, e.g. `ipam/prefixes` → `Prefixes` with `available_ips`/`available_prefixes` properties returning a `DetailEndpoint`). `DetailEndpoint.list()` reuses `RecordSet` — its plain-list branch handles non-paginated detail routes. Import order matters: endpoint → models → response; response only TYPE_CHECKING-imports the others.
