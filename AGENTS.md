...
Typed endpoint hints live in [apps.py](src/aiopynetbox/apps.py) - GENERATED by `scripts/generate_endpoints.py` from a NetBox OpenAPI schema (default demo.netbox.dev), never edited by hand; a weekly workflow (regenerate-endpoints.yml) opens a PR on drift. The per-app subclasses hold bare class annotations (static analysis only, no runtime attributes); `App.__getattr__` remains the real mechanism, so unlisted endpoints still work. `APP_CLASSES` maps app name -> class and tests assert every annotation resolves.
