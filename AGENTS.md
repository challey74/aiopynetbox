...
Typed endpoint hints live in [apps_generated.py](src/aiopynetbox/apps_generated.py) and [hints_generated.pyi](src/aiopynetbox/hints_generated.pyi) - both GENERATED by `scripts/generate_endpoints.py` from a NetBox OpenAPI schema (default demo.netbox.dev), never edited by hand; a weekly workflow (regenerate-endpoints.yml) opens a PR on drift. The per-app subclasses hold bare class annotations (static analysis only, no runtime attributes); `App.__getattr__` remains the real mechanism, so unlisted endpoints still work. hints_generated.pyi is stub-only (never imported at runtime, hence pyright's reportMissingModuleSource is disabled): per-endpoint TypedDicts (values Any, name-completion only) feed Unpack overloads on filter/get/count/create, each with a **kwargs: Any fallback overload so cf_* filters, lookup expressions, and plugin params stay legal; lookup variants (__) and cf_* params are excluded at generation. `APP_CLASSES` maps app name -> class and tests assert every annotation resolves and every hint class exists.
